...
import pandas as pd  # 引入 pandas 確保資料庫總覽 100% 穩定
...
view_mode = st.segmented_control("主看板視角切換：", options=["🗓️ 月份大看板", "📋 條列式清單檢視"], default="🗓️ 月份大看板")
st.divider()
...
with col_left:
    ...
    with col_t2: 
        end_time = st.time_input("結束時間", time(10, 0))
    ...
    if st.button("確認加入行事曆", type="primary", use_container_width=True):
        if event_title: 
            st.success(f"🎉 行程【{event_title}】已成功寫入暫存！")
        else: 
            st.error("請填寫行程主旨！")
...
with col_center:
    ...
    mock_events = [
        ...
        {"title": "⚠️ 繳交 PhD 研究計畫書草稿", "date": "2026-05-20", "color": "#D32F2F", "tag": "#緊急"}
    ]
    
    # 如果使用者在左側輸入了新行程，動態加到看板最上方展示（增加互動感！）
    if event_title:
...
with col_right:
    ...
    with st.expander("🛠️ 自動發信核心參數設定", expanded=True): # (API 6)
    ...
    # 修正點：使用明確定型的 Pandas DataFrame，避免 Streamlit 渲染 DateColumn 時報錯
    df_data = pd.DataFrame([
        {"行程名稱": "AI 部署班", "日期": pd.to_datetime("2026-05-18"), "標籤色彩": "#E65100", "已發信": True},
        {"行程名稱": "PhD 計畫書", "日期": pd.to_datetime("2026-05-20"), "標籤色彩": "#D32F2F", "已發信": False},
    ])
    ...
    def show_announcement():
        ...
        st.write(f"當前登入權限組別：**{calendar_group}**")
        
    if st.button("📢 查看課堂專案公告", use_container_width=True): 
        show_announcement()
